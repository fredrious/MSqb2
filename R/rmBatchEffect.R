...
rmBatchEffect <- function(dt,
  ...
  if (row.p[1] == "auto") row.p <- intersect(c("Protein", "Peptide", "Feature"), names(dt))
  if (col.p[1] == "auto") col.p <- names(pheno[, -"Fraction"])
  
  reftb <- dt[, ..col.p] %>% setorderv(., col.p) %>% unique() %>% 
    .[, rr := do.call(paste, c(.SD, sep = sp)), .SDcols = col.p] %>% .[order(rr)]
  
  
  ## long to wide
  mt <- MSqb2:::.l2w(
    dl = dt, col.p = col.p, row.p = row.p, val = val, sp = sp,
  ...
  ## wide to long
  cln.dt <- MSqb2:::.w2l(dw = cln.mat, col.p = col.p, row.p = row.p, val = val)
  
  ## merge with original data
  cln.dt %<>% merge(dt[, !..val], ., by = c(row.p, col.p))
  
  
  if (toupper(batch.corr.method) == "SVA") {
    list(cln.dt = cln.dt, svobj = svobj) %>% return()
  } else {
     return(cln.dt)
  }
}
