...
#'
#' @details The `impute_missing_values` function offers several methods for imputing missing data, depending on the nature of the missingness and the experimental design. For in-depth details please refere to vignette of the packages msImpute and imputeLCMD.
#'
#' \itemize{
...
#'
#' @importFrom magrittr %>%
#' @importFrom imputeLCMD impute.QRILC impute.wrapper.KNN impute.wrapper.MLE impute.MinProb impute.MinDet impute.ZERO impute.MAR.MNAR
#' @importFrom msImpute msImpute
#' @export
...
impute_missing_values <- function(dat,
  ...
  mat <- dat[, c(..row.p, ..col.p, ..val)] %>%
    .l2w(dl = mat, col.p = col.p, row.p = row.p,
  ...
  # wide to long
  imp.mat <- .w2l(dw = imp.mat, col.p = col.p, row.p = row.p, val = val)
  return(imp.mat)
}
